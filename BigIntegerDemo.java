import java.math.BigInteger;


public class BigIntegerDemo 
{

  public BigInteger one = new BigInteger("1");
  public static final int NUM_BITS = 2048; 

  public BigInteger ModExp( BigInteger M, BigInteger e, BigInteger n, 
                            BigInteger Mbar, BigInteger xbar)
  {
    for (int i=NUM_BITS-1; i>=0; i--)
    {
      // square
      xbar = montMult(xbar,xbar,n);

      // multiply   
      if (e.testBit(i)) // if (e.bit(i) == 1)
        xbar = montMult(Mbar,xbar,n);
    }
    // undo montgomery residue transformation
    return montMult(xbar,one,n);
  }

  
  public BigInteger montMult(BigInteger X, BigInteger Y, BigInteger M)
  {
    BigInteger S = new BigInteger("0"); 
    
    for (int i=0; i<NUM_BITS; i++)
    {
      // add product of X.get_bit(i) and Y to partial sum
      if (X.testBit(i))
        S = S.add(Y); // S += X[i]*Y

      // if S is even, add modulus to partial sum
      if (S.testBit(0))  
        S = S.add(M);     

      // rightshift 1 bit (divide by 2)
      S = S.shiftRight(1);
    }

    // bring back to under 1024 bits by subtracting modulus
    if (S.compareTo(M) >= 0)
      S = S.subtract(M);

    return S;
  }


  public static void main(String[] args) 
  {
    BigIntegerDemo demo = new BigIntegerDemo();

    int testcase = 0;

    String Mstr,estr,nstr,Mbarstr,xbarstr;

    System.out.println("NUM_BITS = " + demo.NUM_BITS);

    switch (demo.NUM_BITS) 
    {
      case 1024:
        Mstr = "";
        estr = "";
        nstr = "";
        Mbarstr = "";
        xbarstr = "";
        // Pad exponent and modulus with leading ones 
        while ( estr.length() < 256 ) { estr = "1" + estr; }
        while ( nstr.length() < 256 ) { nstr = "1" + nstr; }
        // Pad message with leading zeros 
        while ( Mstr.length() < 256) { Mstr = "0" + estr; }
        break;

    case 2048:
        Mstr = "";
        estr = "";
        nstr = "";
        Mbarstr = "";
        xbarstr = "";
        // Pad exponent and modulus with leading ones 
        while ( estr.length() < 256 ) { estr = "1" + estr; }
        while ( nstr.length() < 256 ) { nstr = "1" + nstr; }
        // Pad message with leading zeros 
        while ( Mstr.length() < 256) { Mstr = "0" + estr; }
        break;

      default:
        Mstr = "";
        estr = "";
        nstr = "";
        Mbarstr = "";
        xbarstr = "";
        System.err.println("ERROR: Please supply valid modulus bit length (1024, 2048, etc...)");
        System.exit(1);
        break;
    }

    // Declare constants 
    BigInteger M = new BigInteger(Mstr,16);
    BigInteger e = new BigInteger(estr,16);
    BigInteger n = new BigInteger(nstr,16);

    // radix
    BigInteger r = new BigInteger("2");
    r = r.pow(demo.NUM_BITS);
    
    // compute Mbar 
    BigInteger Mbar = M.multiply(r);
    Mbar = Mbar.mod(n);

    // compute xbar 
    BigInteger xbar = r.mod(n);

    // Compute actual solution 
    BigInteger sol = M.modPow(e,n);

    // Print results 
    System.out.println("\nActual solution = "+sol);
    System.out.println("\nInitial Values:");
    System.out.println('\t' + "M = 0x" + M.toString(16));
    System.out.println('\t' + "e = 0x" + e.toString(16));
    System.out.println('\t' + "n = 0x" + n.toString(16));
    System.out.println("\nComputed Values:");
    System.out.println('\t' + "Mbar (" + Mbar.bitLength() + " bits) = 0x" + Mbar.toString(16));
    System.out.println('\t' + "xbar (" + xbar.bitLength() + " bits) = 0x" + xbar.toString(16));

    // perform modular exponentiation 
    BigInteger mySol = demo.ModExp(M,e,n,Mbar,xbar);
    System.out.println("\nComputed Montgomery Result: " + mySol);

    if (!sol.equals(mySol))
        System.out.println("\nFAILURE: Montgomery solution does not match modexp solution!");
    else
    {
        System.out.print("\nPlaintext = ");
        byte[] byteArray = mySol.toByteArray();
        for( int i = 0 ; i < byteArray.length ; i++ )
            System.out.format("%c",byteArray[i]);
        System.out.print("\n********SUCCESS********!\n");
    }
  }
}

